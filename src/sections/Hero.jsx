  ...
  return (
    <div className="relative w-full overflow-hidden shadow-xl">
      <img
        ...
        loading="lazy"
        alt="Background"
        className="h-[60vh] w-full object-cover object-center sm:h-[70vh] lg:h-[80vh]"
      />
      <div className="hero-opacity absolute inset-0 bg-black"></div>

      <div className="absolute inset-0 flex items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl text-center text-white">
          <h1 className="text-xl font-semibold tracking-tight sm:text-2xl lg:text-3xl xl:text-5xl">
            Unlock the Magic of <span className="text-brand-6">Reading</span>
          </h1>
          <h2 className="mt-2 text-lg font-normal tracking-tight sm:text-xl lg:text-2xl xl:text-3xl">
            Where Every Page Holds a New Adventure
          </h2>
          <p className="mt-4 max-w-xl text-sm leading-5 text-hero-text sm:text-base md:text-lg md:leading-7 lg:max-w-2xl">
            Welcome to InstantMart, your haven for book lovers and avid readers alike.
            Dive into a world where imagination knows no bounds and every book
            is a portal to new experiences. From timeless classics to the
            latest bestsellers, our curated collection awaits your discovery.
          </p>
          <Link to="/products" className="mt-6 inline-block">
            <button className="group flex items-center justify-center gap-2 rounded-full bg-brand-6 px-4 py-2 text-sm font-semibold uppercase text-brand-0.5 transition-colors duration-300 hover:bg-brand-7 hover:text-brand-0.5 focus:bg-brand-7 focus:outline-none focus:ring focus:ring-brand-7 focus:ring-offset-2 disabled:cursor-not-allowed">
              Explore Now
              <HiArrowLongRight className="transform transition-transform duration-300 group-hover:translate-x-0.5" />
            </button>
          </Link>
